...
class Water_Phantom:
    def __init__(self, dicom_image_file, r=INNER_RADIUS):
        """
    ...
    def get_water_body_boundary(self):
        ...
        # 20200509,取ind找边缘
        rads = np.array(rads)
        ind = np.where(abs(rads - self.INNER_RADIUS + 3) < 4)  # 筛选出计算出的半径与实际水模体半径相仿的，即为水模体边界
        print(rads, ind, self.INNER_RADIUS, ind[0])
        self.water_body_boundary = lb == (ind[0][-1] + 1)  # scipy.argmin(rads)
    ...
    def water_roi_new(self):
        ...
        air_pixels = scipy.zeros((watery1 - watery2, waterx2 + 512 - waterx1))
    ...
    def grid_generation(self, box_size):
        ...
        # generate the indices of the boxes
        pixel_size = self.dataset.PixelSpacing[0]
        print(box_size * pixel_size)
        if None:  # abs(box_size*pixel_size -7)<1:

            n = int((r) * sqrt(2) / 2 / box_size) * 2
            # print(n,r-buffer_size,(r-buffer_size)/box_size,10000009)
            # ys,xs = np.ogrid[-n / 2*box_size:n / 2*box_size:n, -n / 2*box_size:n / 2*box_size:n]
            # mask = (xs ) ** 2 + (ys ) ** 2 <= (r-buffer_size) ** 2
            # xs1,ys1 = scipy.where(mask)
            # print(xs,ys)
            ...
            for i in ys.flatten():
                for j in xs.flatten():
                    if i ** 2 + j ** 2 <= ((r - buffer_size)) ** 2:  # /np.sqrt(2)

                        ysnew.append(i + int(round(yc)))
                        xsnew.append(j + int(round(xc)))
            ys = np.array(ysnew)
            xs = np.array(xsnew)
...
if __name__ == '__main__':
    ...
    # fname = "D:/motituxiang/motituxiang/冠状和矢状-1\Z01"
    # fname = "D:\\医学模体图像\\三维轴扫和儿童\\Z20"
    fname = "D:\\医学模体图像\\0901儿童模94多条件性能\\80-200\\Z526"
    ds = dicom.read_file(fname)

    if 1:  # for f in os.listdir(pn):
        print(fname)
        # fname = os.path.join(pn, f)
        test = Water_Phantom(fname, r=43)
        av, noise = test.water_roi()
        homogeneity = test.homogeneity()
        ##        homogeneity = test.homogeneity()
        sz1 = 2  # mm                                                                       #这是个什么参数呢？
        sz = int(round(sz1 / ds.PixelSpacing[0]))
        lcd = test.calculate_lcd(sz, show_hist=True)  # 计算lcd值

        sz1 = 3  # mm                                                                       #这是个什么参数呢？
        sz = int(round(sz1 / ds.PixelSpacing[0]))
        lcd = test.calculate_lcd(sz, show_hist=True)
